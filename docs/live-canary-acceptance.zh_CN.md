...
## 0.7.1 发布后验证（2026-09-14）

- **发布提交处的源码等价性：** 标签 `v0.7.1`、marketplace 跟踪副本、已安装缓存均指向 `1314155`。发布时 `origin/main` 同为 `1314155`，其后仅增加本记录。
- **`1314155` 处的安装产物一致性：** 仓库树与已安装副本的全部 438 个受跟踪文件逐字节一致；安装清单为 0.7.1，且不存在残留的 0.7.0 安装。
- **安装副本自检：** 安装副本的发布校验器报 0.7.1；白名单测试在安装副本上通过；`stitch.withgoogle.com` 被接受，相似域名被拒绝。
- **Marketplace 刷新：** 第一次 `marketplace upgrade` 因瞬时 `fatal: early EOF` 失败并把跟踪副本留在 `e86b8b0`；重试后升到 `1314155`，并重装使缓存与跟踪副本一致。
- **持续集成：** `Validate` workflow 在 `main` 与 `v0.7.1` 标签上均成功。
- **新任务工具暴露：被环境阻塞，未验证。** 新建 ephemeral 任务中模型只看到自带的 `mcp__cua_repl__*`，十个已配置的用户与插件 MCP 服务全部未暴露，因此无法调用任何 Stitch 工具。这不是本插件的问题：同批探针对**任何**用户配置服务都报无工具，进程 stderr 与当天早先成功那次逐字节相同，`codex mcp list` 仍列出全部服务，`codex mcp get stitch` 指向已安装的 0.7.1 代理，直接驱动该代理返回完整的 17 工具清单。重启 Codex 后需重跑本项；0.7.0 的暴露检查在当天早些时候是通过的。

## 0.7.0 发布后验证（2026-09-14）

- **发布提交处的源码等价性：** 标签 `v0.7.0`、本次刷新后的 marketplace 跟踪副本、已安装缓存均指向 `e86b8b0`。发布时 `origin/main` 同为 `e86b8b0`，其后仅增加了本验证记录，因此标签与已安装副本仍是该发布版本的基准。
- **`e86b8b0` 处的安装产物一致性：** 仓库树与已安装副本的全部 437 个受跟踪文件逐字节一致；安装清单为 0.7.0，且不存在残留的 0.6.x 安装。
- **新任务工具暴露：** 新建 ephemeral 任务成功暴露并调用了 `mcp__stitch__list_projects`，且 `STITCH_API_KEY` 未设置，说明凭据来自受限的用户配置；调用经由插件自带的 stdio 代理。
- **持续集成：** `Validate` workflow 在 `main` 与 `v0.7.0` 标签上均成功完成。
- **设备栅栏对已记录回退稿的判定：** 把所报项目中观测到的平板屏幕（`DESKTOP` 2560×2048）交给栅栏、目标为 `TABLET` 768×1024 时，三张全部失败；而已交付的移动屏幕（`MOBILE` 780×1768 对 390×884）以 scale 2 通过。

## 0.6.1 发布后验证（2026-09-14）
