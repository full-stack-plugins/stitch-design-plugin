...
>
> **适用版本**：0.5.4 · **状态**：立即发布目标 · **事实核验日期**：2026-09-14
...
sequenceDiagram
    ...
    participant P as 插件
    U->>C: 添加市场并安装插件
    C->>M: 为 v0.5.4 解析 main
    M-->>C: stitch-design 0.5.4 发布目标
...
stateDiagram-v2
    ...
    需要设置 --> 打开向导
    打开向导 --> 已保存: 本地请求合法
    已保存 --> 已重启: 启动新 Codex 进程
    已重启 --> 已验证: list_projects 返回列表或空列表
    已验证 --> 可用
```
...
**文档版本**：2.4.0 · **状态**：已对齐 0.5.4 立即发布目标 · **最后更新**：2026-09-14
