...
>
> **适用版本**：0.7.7 · **状态**：发布候选 · **事实核验日期**：2026-09-14
...
| 能力 | 当前 | 目标 | 差距 |
|:---|:---|:---|:---|
| Skill 目录 | 已发布并验证 43 个 Skill | 不变 | 无 |
| MCP 工具面 | 17 个工具：15 个来自 Google Stitch 服务器，外加 2 个本地资产工具 | 不变 | 无 |
| 凭据处理 | 本地三步向导，写入受限的当前用户配置 | 不变 | 无 |
| Harness 交付 | 带证据门禁的本地交互式控制器路径 | 不变 | 无 |
| Portable Agent Plugins 根清单 | 尚未迁移 | 已迁移 | 迁移门禁为有意保持开放 |
| Universal 公共 Plugins Directory | 未发布 | 已发布 | 需要独立提交与远端 HTTPS MCP 审查 |
| Provider 与资产真机 smoke | 已在本机通过 | 不变 | 未远程运行，且不构成 Harness 验收 |
| ChatGPT 网页路径 | 实验性 | 不变 | 已观察到连接停滞；受支持的界面是本地 Codex |

没有任何一行超出其证据。两行发布相关内容不在本仓库控制范围内，因此记为差距，而不是带日期的计划。
...
sequenceDiagram
    ...
    participant P as 插件
    U->>C: 添加市场并安装插件
    C->>M: 为 0.7.7 候选解析 main
    M-->>C: stitch-design 0.7.7 候选
...
| 预算 | 值 | 理由 |
|:---|:---|:---|
| 设置触发冷却 | 600 秒 | 凭据缺失不能变成弹窗循环 |
| 凭据刷新 | 恰好一次 401 刷新 | 第二次失败必须暴露，而不是继续重试 |
| 工具面 | 17 个工具 | 15 个上游加 2 个本地，且本地那一对绝不冒充上游 |
| 下载白名单 | Stitch 自有 web 域加上经审核的主机 | 相似域名与不安全 URL 会被拒绝 |
| 运行批准 | 仅显式人工批准 | 门禁不能被模型决策穿过 |
| 离线套件 | 243 个测试，不访问网络 | 测试必须能在运行插件的同一环境里跑 |
...
**文档版本**：2.7.7 · **状态**：已对齐 0.7.7 发布候选 · **最后更新**：2026-09-14
