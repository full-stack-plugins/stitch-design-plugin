...
> Design, edit, inspect, and convert Google Stitch projects from Codex through 40 workflow-focused Agent Skills.
...
Codex
  ...
  ▼
Stitch Design
  ├─ 40 Skills: routing, safety, design, conversion
  ├─ local setup UI: get key → save → open Codex
  └─ MCP mapping: STITCH_API_KEY → X-Goog-Api-Key
                         │
                         ▼
                 Google Stitch MCP
```
...
The page binds only to `127.0.0.1`, loads no external assets, uses CSRF and Origin checks, never logs the key, and clears the input after every response. The credential file is permission-restricted but is not an encrypted system vault. See [Getting started](docs/getting-started.zh-CN.md) and [Privacy](PRIVACY.md).
...
2. The user-scoped Stitch Design credential file.
3. First-use setup.

Default credential locations:

- Unix: `$XDG_CONFIG_HOME/stitch-design/credentials.json` or `~/.config/stitch-design/credentials.json`.
...
Release 0.4.0 evidence: 16 automated tests passed, 40 Skills validated, the public Marketplace installation resolved 0.4.0, and the setup page was visually checked at 390×884, 768×1024, and 1280×1024. Live Stitch availability remains an external dependency.
...
The 39 upstream Skill snapshots are based on `full-stack-skills/stitch-skills` commit `62ef81825ad6ddc85bb6b8426e65b1a9d07d109b`; `stitch-local-setup` is plugin-specific. Official adapted material traces to `google-labs-code/stitch-skills` commit `0337446dadde6f8c94210444e2aa9d546126480f`.
