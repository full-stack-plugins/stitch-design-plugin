...
- `enhance` → ImageGen 美术增强 → OCR/业务门禁 → 回灌 Stitch → 可编辑性验证 → 双图对比 → 用户批准 → 正式归档。
...
6. Stitch 原稿验收后进入 `AWAITING_ART_DECISION`。向用户明确询问是否二次图片生成，并执行 `art-decision --decision enhance|keep_stitch|cancel --source user`；模型、自动评分和旧请求均不能代替本次选择。
7. 仅 `enhance` 分支调用 ImageGen；结果先过尺寸、OCR 和业务断言，不通过不回灌。
8. 回灌结果必须含重新下载的 HTML 和渲染图。探针编辑需保存 before/edited/restored 三份 HTML 与三张 render；edited 哈希必须变化，restored HTML/render 哈希必须分别与 before 完全相等。`keep_stitch` 分支对已接受的 Stitch 源 HTML/render 执行同一探针，不伪造 ImageGen 或回灌证据。
9. `enhance` 分支的双图阶段从 receipts 读取已接受的 ImageGen art render 与最终 Stitch render，禁止接受调用者另传图片；生成 `side-by-side.png`、`overlay.png`、`diff-heatmap.png`，并把两张源图写入 evidence `source_artifacts` 和 receipt `inputs`。未接受 visual receipt 时可原子替换完整三图且失败回滚；一旦通过的 visual receipt 已存在，即使状态更新曾中断，也禁止覆盖。布局分数和五个视觉维度通过后进入 `AWAITING_USER_APPROVAL`。
10. 用户批准文件必须绑定该分支的必需产物：`enhance` 绑定本次所见 HTML、两张最终图和比较图哈希；`keep_stitch` 绑定已接受的 Stitch HTML/render。文件变化会使批准失效。
11. `archive` 只接受 `APPROVED`。若移动对话中已经引用的图片，必须保留可读的相对软连接。
12. 若 archive 已完成原子发布、但 run 状态持久化中断，重新执行同一 `archive --run`；Harness 先复验已发布收据链、批准和哈希，再只补写 `ARCHIVED` 状态。
...
python scripts/stitch_harness.py art-decision --project /absolute/project --run RUN_ID --decision enhance --source user
